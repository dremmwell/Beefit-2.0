...
async function GraphWrapper( {user} : {user : User}) {
  ...
  const startDate : Date = new Date(today.getFullYear(), today.getMonth(), today.getDate() - 365/2)
  ...
  const dataOrdered : DayData[] = data.sort((a,b) =>  a.date.getTime() - b.date.getTime() )

  return (
    <div className="flex flex-col xl:flex-row gap-4 xl:mt-2 overflow-scroll no-scrollbar">
        <div className="flex flex-col gap-4 w-full">
            <HomeGraph data={dataOrdered} />
        </div>
    </div>
  )
}
